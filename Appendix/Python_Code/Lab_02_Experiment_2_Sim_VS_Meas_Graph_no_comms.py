...
plt.plot(voltageIN, np.array(Current) * 1e3, label="Simulated $I_D$", color="g", linestyle="-", linewidth=2)
plt.plot(Mes_VDs, np.array(Mes_I) * 1e3, label="Measured $I_D$", color="r", linestyle="-", linewidth=2)
plt.xlabel('Voltage (V)')
plt.ylabel('Current (A)')
plt.title('Experiment 2: Simulated vs Measured Data')
plt.legend()
plt.grid()
plt.show()