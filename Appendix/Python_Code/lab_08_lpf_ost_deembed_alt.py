...
lpf_deembedded = deembed_osp(dut, open_standard, short_standard, thru_standard)
lpf_deembedded.write_touchstone('lpf_deembedded.s2p')

plt.figure(figsize=(12, 8))

plt.subplot(2, 1, 1)
plt.title('LPF - OST De-Embedding - S21 - Transmission')
dut.plot_s_db(m=0, n=1, label='Original')
lpf_deembedded.plot_s_db(m=0, n=1, label='De-embedded')
plt.grid(True)
plt.legend()

plt.subplot(2, 1, 2)
plt.title('LPF - OST De-Embedding - S11 - Reflection')
dut.plot_s_db(m=0, n=0, label='Original')
lpf_deembedded.plot_s_db(m=0, n=0, label='De-embedded')
plt.grid(True)
plt.legend()

plt.tight_layout()
plt.savefig('lpf_deembedded_comparison.png')
plt.show()